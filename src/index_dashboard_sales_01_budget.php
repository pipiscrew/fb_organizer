...
if (isset($_GET["month"]) && $_GET["month"]==1)
{
	...
	for($m= 1 ; $m < 13 ; $m++)
	{
		...
		for( $i= 0 ; $i <= sizeof($chart_db_users)-1 ; $i++ )
		{
			...
			$total_tmp = getScalar($db,"select ifNull(FORMAT(sum(offer_total_amount),0),0)  from offers left join clients on clients.client_id = offers.company_id where clients.is_lead=0 and offer_seller_id = " . $chart_db_users[$i]['user_id'] . " and is_paid=1 and is_paid_when BETWEEN '".$start." 00:00:00' AND '".$end." 23:59:59'",null);
			$total_tmp = str_replace(",","",$total_tmp); 

			$col_vals[] =  (float) $total_tmp;
		}
...
else 
{
	...
	for($m= 0 ; $m < 7 ; $m++)
	{
		...
		{
			//query with date between depends on $m variable
			$total_tmp = getScalar($db,"select ifNull(FORMAT(sum(offer_total_amount),0),0)  from offers left join clients on clients.client_id = offers.company_id where clients.is_lead=0 and offer_seller_id = " . $chart_db_users[$i]['user_id'] . " and is_paid=1 and is_paid_when BETWEEN '".$day_str." 00:00:00' AND '".$day_str." 23:59:59'",null);
			$total_tmp = str_replace(",","",$total_tmp); 
			...
			<button onclick="refresh_grid()" class="btn btn-primary" style="margin-top:25px">Refresh</button>
				...
				<div class="row">					
					<table id="pay_pend_tbl"
						data-url="index_dashboard_sales_01_budget_pagination.php"
						...
						<thead>
							<tr>
								...
								<th data-field="service"  data-formatter="serviceFormatter" data-sortable="true" data-visible="true">Service Ads Duration</th> 
								...
								<th data-field="gen_total" data-formatter="proposalFormatter" data-sortable="true">Invoice Amount</th>
								<th data-field="offer_seller_name" data-sortable="true">Seller</th>
								<th data-field="mrkt_campaigner" data-sortable="true">Campaigner</th>
								<th data-field="is_paid_when" data-sortable="true" data-visible="true">Contract Paid</th> 
								<th data-field="invoice_sent_when" data-sortable="true" data-visible="true">Invoice Sent</th> 								
							</tr>
						</thead>

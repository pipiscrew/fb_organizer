...
//	{
		...
		$col_vals[] = (int) getScalar($db,"select sum(offer_total_amount) from offers where offer_seller_id=".$_SESSION['id']." and is_paid = 1 and offer_proposal_date between '".$start."' and '".$end."'",null);
//	}
...
?>
   ...
   var x = <?= json_encode($chart_row_setup);?> ;
   console.log(x);
			...
			<button onclick="refresh_grid()" class="btn btn-primary" style="margin-top:25px">Refresh</button>
				...
				<div class="row">					
					<table id="pay_pend_tbl"
						data-url="tab_dashboard_seller_paid_list_pagination.php"

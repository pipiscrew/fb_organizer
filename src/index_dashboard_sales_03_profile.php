...
if (isset($_GET["month"]) && $_GET["month"]==1)
{
	...
	for($m= 1 ; $m < 13 ; $m++)
	{
		...
		for( $i= 0 ; $i <= sizeof($chart_db_users)-1 ; $i++ )
		{
			...
			$total_tmp = (int) getScalar($db,"SELECT count(client_id) FROM `clients` where owner= " . $chart_db_users[$i]['user_id'] . "  and (profile_sent_when BETWEEN '".$start."' AND '".$end."')",null);
			$col_vals[] =  $total_tmp;
		}
...
else 
{
	...
	for($m= 0 ; $m < 7 ; $m++)
	{
		...
		{
			//query with date between depends on $m variable
			$total_tmp = (int) getScalar($db,"SELECT count(client_id) FROM `clients` where owner= " . $chart_db_users[$i]['user_id'] . "  and (profile_sent_when BETWEEN '".$day_str."' AND '".$day_str."')",null);
			...
			<button onclick="refresh_grid()" class="btn btn-primary" style="margin-top:25px">Refresh</button>
				...
				<div class="row">					
					<table id="pay_pend_tbl"
						data-url="index_dashboard_sales_03_profile_pagination.php"

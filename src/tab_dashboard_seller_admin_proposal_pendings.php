...
for( $m= 1 ; $m <= 12 ; $m++ )
{
	...
	for( $i= 0 ; $i <= sizeof($chart_db_users)-1 ; $i++ )
	{
		...
		$col_vals[] = (int) getScalar($db,"select count(offer_id) from offers where offer_seller_id=".$chart_db_users[$i]['user_id']." and approval_user_date is null and offer_proposal_date between '".$start."' and '".$end."'",null);
	}
			...
			<button onclick="refresh_grid()" class="btn btn-primary" style="margin-top:25px">Refresh</button>
				...
				<div class="row">					
					<table id="pay_pend_tbl"
						data-url="tab_dashboard_seller_admin_proposal_pendings_pagination.php"

...
//log
	...
	if ($ret_val == "isnew")
	{
		...
		header("Location: tab_clients_details.php?showcalls=1&id=".$customer_id);
	}
	else 
	{
		write_log($db, 4, "Client ".$_POST['client_name']." updated by seller ".$_SESSION['u'], $_POST['clientsFORM_updateID'], $_SESSION['id']);
		
		header("Location: tab_clients_details.php?id=".$_POST['clientsFORM_updateID']);
		//header("Location: tab_clients.php?$ret_val=1");
	}
}
	//header("Location: tab_clients.php?$ret_val=1");
else

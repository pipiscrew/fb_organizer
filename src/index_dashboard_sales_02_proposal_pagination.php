...
$sql="select service_ends >= CURDATE() as is_ended, service_starts > CURDATE() as is_start,offer_company_name,is_deleted,offer_seller_name,clients.is_lead as is_lead,CONCAT('rec_guid=',rec_guid) as url, offer_company_manager_name,offer_telephone,company_id,offer_id,FORMAT(gen_total,2, 'de_DE') AS gen_total,DATE_FORMAT(offer_proposal_date,'%d-%m-%Y') as offer_proposal_date,DATE_FORMAT(rec_guid_last_viewed_when,'%d-%m-%Y %H:%i') as seen
 from offers 
left join clients on clients.client_id = offers.company_id
where 1=1 {$where}";
$count_query_sql = "select count(*) from offers where 1=1 {$where}";

$total_query_sql = "select FORMAT(sum(gen_total),2, 'de_DE') as gen_total from offers where 1=1 {$where}";
